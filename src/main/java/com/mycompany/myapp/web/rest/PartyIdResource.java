...
public class PartyIdResource {
    ...
    public List<PartyId> getAllPartyIds(@RequestParam(required = false) String filter) {
        if ("party-is-null".equals(filter)) {
            log.debug("REST request to get all PartyIds where party is null");
            return StreamSupport
                .stream(partyIdRepository.findAll().spliterator(), false)
                .filter(partyId -> partyId.getParty() == null)
